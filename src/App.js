...
function App(props) {
  ...
  useEffect(() => {
    fetch(
      `https://pixabay.com/api/?key=41515511-97188783ec5b670a2e590b766&q=${term}&image_type=photo&pretty=true`
    )
      .then((res) => res.json())
      .then((data) => {
        setImages(data.hits);
      ...
      <>
          <div className="w-screen flex items-center justify-center h-40 bg-slate-400 fixed inset-0 z-50">
              <input
                  ...
                  name="search"
                  id=""
                  placeholder="Recherchez votre image..."
          ...
          {!isLoading && images.length === 0 && (
              <h1 className="text-center text-4xl mt-44">Aucun résultat</h1>
          )}
          ;
          {isLoading ? (
